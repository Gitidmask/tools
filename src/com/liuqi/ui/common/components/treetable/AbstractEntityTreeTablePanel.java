...
package com.liuqi.ui.common.components.treetable;
 ...
 * @version V1.0
 **/
public abstract class AbstractEntityTreeTablePanel extends AbstractTreeTablePane<Entity> {
    ...
    private void setMouseSelectAction() {
        ...
        this.getTableView().setOnMouseDragEntered(event -> {
            tableMouseSelection.isPressed = true;
        });

        this.getTableView().setOnMouseDragged(event -> {
            if (tableMouseSelection.isPressed) {
                System.out.println(event.getSource());
            }
        });

        this.getTableView().setOnMouseDragReleased(event -> {
    ...
    private void setCopyAction() {
        this.getTableView().setOnKeyPressed(event -> {
           if (event.getCode() == KeyCode.C && event.isControlDown()) {
               ObservableList<TreeTablePosition<Entity, ?>> selectedCells = this.getTableView().getSelectionModel().getSelectedCells();
    ...
    public void initToolBar(ToolBar toolBar) {
        ...
        textField.setOnKeyReleased(event -> {
            if (KeyCode.ENTER == event.getCode()) {
                ...
                TreeItem<Entity> selItem = getTableView().getSelectionModel().getSelectedItem();
                Entity parentEntity = null;

                if (null != selItem) {
                    if (null != selItem.getParent() && !selItem.getParent().equals(getTableView().getRoot())) {
